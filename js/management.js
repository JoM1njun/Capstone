...
function renderPage() {
  ...
  currentItems.forEach((item) => {
    ...
    itemDiv.innerHTML = `
            ...
            <span contenteditable="false" style="color: ${parseInt(item.status) === 1 ? 'green' : 'red'};">${parseInt(item.status) === 1 ? "ON" : "OFF"}</span>
        `;
        ...
        <div class="detail-header">
            ...
            <div class="button_group">
                ...
                <div class="settings-menu hidden">
                    ...
                    <button type="button" onclick="saveDetail('${item.id}', [])">저장</button>
  ...
  paragraphs.forEach((p, index) => {
    ...
    if (strong.includes("종류 :")) {
      ...
      <div class="radio-group">
        <label>
          <input type="radio" name="type" value="소화기" ${currentType === "소화기" ? "checked" : ""}>
          소화기
        </label>
        <label>
          <input type="radio" name="type" value="소화전" ${currentType === "소화전" ? "checked" : ""}>
          소화전
        </label>
        <label>
          <input type="radio" name="type" value="AED" ${currentType === "AED" ? "checked" : ""}>
          AED
        </label>
      </div>
      `;
      ...
      try {
        ...
        const [year, month, day] = dateStr.match(/(\d+)년\s*(\d+)월\s*(\d+)일/).slice(1);
      ...
      p.innerHTML = `<strong>${p.querySelector("strong").textContent}</strong> 
      <input type="text" value="${text}" class="edit-input">`;
    }
  });

  // 저장 버튼 설정 부분 수정
  const saveButton = button.closest(".settings-menu").querySelector("button:nth-child(2)");
...
async function saveDetail(itemId, originalData) {
  ...
  // radio button 값 가져오기
  const selectedType = detailBody.querySelector('input[name="type"]:checked')?.value;
  if (!selectedType) {
    alert('종류를 선택해주세요.');
    return;
  }
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/management/${itemId}`, {
      method: 'PUT',
      headers: {
...
//       p.innerHTML = `<strong>${strong.textContent}</strong> ${inputs[index].value}`;
//   });
